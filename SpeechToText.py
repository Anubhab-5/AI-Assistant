...
# Define the HTML sode for the speech recognition interface.
...
# Get the current working derectory.
...
# Define the path for tempery files.
...
# Function to modify a query to ensure proper punctuation and formating.
def QueryModifier(Query):
    new_query = Query.lower().strip()
    query_words = new_query.split()
    question_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's", "where's", "how's", "can you"]

    # Check if the query is a question and add a question mark if nessary.
    if any(word + " " in new_query for word in question_words):
        ...
        # Add a period if the query is a question.
        if query_words[-1][-1] in ['.', '?', '!']:
            new_query = new_query[:-1] + "."
        else:
            new_query += "."
...
def SpeechRecognition():
    ...
    # Start speech recognition by clickiing the start button.
    ...
    while True:
        try:
            ...
            if Text:
                # Stop recognition text from the HTML output element.
                ...
                if InputLanguage.lower() == "en" or "en" in InputLanguage.lower():
                    ...
                    # If the input language is not English, tranlate the text and return it.
