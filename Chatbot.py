...
# Lode environment variable from the .env file.
...
System = f"""Hello, I am {Username}, You are a very accurate and advanced AI chatbot or assistant named {Assistantname} I made by {Username} which also has real-time up-to-date information from the internet.
...
# Attempt to lde the chat log from a JSON file.
try:
...
def RealtimeInformation():
    ...
    data = f"Please use this real-time information id needed,\n"
    data += f"Day: {day}\nDate: {date}\nMonth: {month}\nYear: {year}\n"
    data += f"Time: {hour} hours :{minute} minutes : {second} seconds.\n"
    return data

# Function to modify the chatbot's response for better formating.
def AnswerModifier(Answer):
    lines = Answer.split('\n') # Split the response into lines.
    non_empty_lines = [line for line in lines if line.strip()] # Remove empty lines.
    modified_answer = '\n'.join(non_empty_lines) # Join the cleaned lines back togather.
...
def ChatBot(Query):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            top_p=1, # Use nucleus sampling control diversity.
        ...
        for chunk in completion:
            if chunk.choices[0].delta.content: # Chunk if there's content in the current chunk.
        ...
        # Handle errors by printing the exception and resettiing the chat log.
        print(f"Error: {e}")
        with open(r"Data\ChatLog.json", "w") as f:
            dump([], f, indent=4)
